...
int dlt645_read_addr(dlt645_t* ctx, uint8_t* addr)
{
    uint8_t addr_bak[6] = { 0 };

    memset(addr, 0xAA, 6);
    memcpy(addr_bak, ctx->addr, 6);
    return dlt645_2007_read_addr(ctx, addr, 1000);
}
 ...
 */
int dlt645_receive_msg(dlt645_t* ctx, uint8_t* msg, uint16_t len, uint32_t code, dlt645_protocal protocal, int timeout_ms)
{
    int msg_len = 0;
    //int i = 0;
    ...
    if(timeout_ms > 0 && ctx->select) {
        if(ctx->select(ctx) == ctx->s) {
            msg_len = ctx->read(ctx, msg, len);
        }
    } else {
        msg_len = ctx->read(ctx, msg, len);
    }
    // DLT645_LOG("dlt645_receive_msg [%d, ", msg_len);
    // for(i = 0; i < msg_len; i++) {
    //     printf("%02X ", msg[i]);
    // }
    // DLT645_LOG("]\n");
...
int dlt645_send_msg(dlt645_t* ctx, uint8_t* msg, int len)
{
    ...
    // DLT645_LOG("dlt645_send_msg [%d, ", len);
    // for(i = 0; i < len; i++) {
    //     printf("%02X ", msg[i]);
    // }
    // DLT645_LOG("]\n");
 ...
 */
int dlt645_read_data(dlt645_t* ctx, uint32_t code, uint8_t* read_data, dlt645_protocal protocal, int timeout_ms)
{
    int rs = -1;
    switch(protocal) {
    case DLT645_1997:
        rs = dlt645_1997_read_data(ctx, code, read_data, timeout_ms);
        break;
    case DLT645_2007:
        rs = dlt645_2007_read_data(ctx, code, read_data, timeout_ms);
        break;
    default:
