...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    do {
        ...
        if (rc == -1) {
            _error_print(ctx, "modbus.c-send_msg.170");
                              ...
                              uint8_t *rsp, int rsp_length)
{
    ...
    if (rsp_length == rsp_length_computed ||
        ...
        _error_print(ctx, "modbus.c-check_confirmation.567");
        rc = -1;
    } else {
        if (ctx->debug) {
            fprintf(stderr,
...
/* Reads IO status */
static int read_io_status_ex(modbus_t *ctx, int function,
                          int addr, int nb, uint8_t *dest, uint8_t bit2byte)
{
    int rc;
    int req_length;
...
/* Same as modbus_read_input_bits but dest is compact, 8bit use 1byte */
int modbus_read_input_bits_compact(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
    int rc;
    ...
    rc = read_io_status_ex(ctx, _FC_READ_DISCRETE_INPUTS, addr, nb, dest, 0);
