...
static void * QmiWwanThread(void *pData) {
    ...
    dbg_log("profile->proxy=%s, cdc_wdm=%s", profile->proxy, cdc_wdm);
    if (profile->proxy[0])
    ...
    dbg_log("QMIThread send event RIL_INDICATE_DEVICE_CONNECTED to main");
    qmidevice_send_event_to_main(RIL_INDICATE_DEVICE_CONNECTED);
    dbg_log("QMIThread enter loop");
    while (1) {
        ...
        dbg_log("QMIThread poll return nevents=%d", nevents);
        for (ne = 0; ne < nevents; ne++) {
            ...
            if (fd == qmidevice_control_fd[1]) {
                dbg_log("event is emit by qmidevice_control_fd[1]=%d", qmidevice_control_fd[1]);
                int triger_event;
            ...
            if (fd == cdc_wdm_fd) {
                dbg_log("event is emit by cdc_wdm_fd=%d", cdc_wdm_fd);
                ssize_t nreads;
