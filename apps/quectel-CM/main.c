...
static void usbnet_link_change(int link, PROFILE_T *profile) {
    ...
    dbg_log("link=%d, call %s", link, link == 0 ? "udhcpc_stop" : "udhcpc_start");
    if (link) {
...
//sudo apt-get remove ModemManager
    ...
    dbg_log("create QMIThread, thread handler is profile->qmi_ops->read=QmiWwanThread()");
    ...
    dbg_log("create QMITHread success");

    dbg_log("read qmidevice_control_fd[0] to get triger_event");
    ...
    dbg_log("read qmidevice_control_fd[0] to get triger_event success");
    ...
    while (1)
    {
        ...
        dbg_log("poll return nevents=%d", nevents);
        for (ne = 0; ne < nevents; ne++) {
            ...
            {
                dbg_log("event is emit by signal_control_fd[1]=%d", signal_control_fd[1]);
                if (read(fd, &signo, sizeof(signo)) == sizeof(signo))
                {
                    dbg_log("read get signo = %d", signo);
                    alarm(0);
                    switch (signo)
                    {
                        case SIG_EVENT_START:
                            dbg_log("case SIG_EVENT_START");
                        ...
                        case SIG_EVENT_CHECK:
                            ...
                            if (IPv4ConnectionStatus ==  QWDS_PKT_DATA_DISCONNECTED && IPv6ConnectionStatus ==  QWDS_PKT_DATA_DISCONNECTED) {
                                dbg_log("call usbnet_link_change");
            ...
            if (fd == qmidevice_control_fd[0]) {
                dbg_log("event is emit by qmidevice_control_fd[0]=%d", qmidevice_control_fd[0]);
                if (read(fd, &triger_event, sizeof(triger_event)) == sizeof(triger_event)) {
                    dbg_log("read get triger_event = %d, 0x%x", triger_event, triger_event);
                    switch (triger_event) {
                        case RIL_INDICATE_DEVICE_DISCONNECTED:
                            dbg_log("case RIL_INDICATE_DEVICE_DISCONNECTED");
                            dbg_log("call usbnet_link_change");
                            usbnet_link_change(0, profile);                            
                            goto __main_quit;
                        break;
                        ...
                        case RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED:
                            dbg_log("case RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED");
                            request_ops->requestRegistrationState(&PSAttachedState);
                        ...
                        case RIL_UNSOL_DATA_CALL_LIST_CHANGED:
                            dbg_log("case RIL_UNSOL_DATA_CALL_LIST_CHANGED");
                            ...
                            dbg_log("send SIG_EVENT_CHECK to main");
                            send_signo_to_main(SIG_EVENT_CHECK);
                        break;
                        ...
                        {
                            dbg_log("case RIL_UNSOL_LOOPBACK_CONFIG_IND");
...
//判断模块工作在什么模式，并设置对应的ops，然后执行qmi_main()
...
int main(int argc, char *argv[])
{
    ...
    dbg_log("user specify logfile=%s, fopen return logfilefp=%p", ctx->logfile, logfilefp);
